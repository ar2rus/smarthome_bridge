    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                int deviceId = Integer.parseInt(a.getOptionValue("d"));
                device = SmarthomeDevice.getByAddress(deviceId);
                if (device == null) {
                    throw new IllegalArgumentException("Unknown device with address: " + deviceId);
                }
...
//                }
        ...
        MulticastConnection connection = null;
        try {
            ...
            if (connection.open()) {
                ...
                if (hexReader.read(firmwarePath)) {
                    ...
                    if (firmwareMaxSize < 0 || firmwareMaxSize >= hexReader.getLength()) {
                        ...
                        final SmarthomeDevice device_ = device;

                        log("Loaded " + hexReader.getLength() + " bytes");
                        SmarthomeMessageFilter bootControlFilter = SmarthomeMessageFilter.sc(device, SmarthomeCommand.BOOT_CONTROL);

                        SmarthomeMessage r = null;
                        if (bootTimeout >= 0) {
                            log("Sending reboot command...");
//                            connection.sendData(device, SmarthomeCommand.REBOOT, null);
                            r = connection.sendDataAndWaitResponse(device, SmarthomeCommand.REBOOT, null, bootControlFilter, bootTimeout);
                        }
                        if (bootTimeout < 0 || r != null) {
                            ...
                            if (bootTimeout < 0 || (r.getData().length == 1 && r.getData()[0] == COMMAND_FIRMWARE_UPDATE_START)) {
                                ...
                                r = connection.sendDataAndWaitResponse(device, SmarthomeCommand.BOOT_CONTROL,
                                ...
                                if (r != null && r.getData().length == 3 && r.getData()[0] == COMMAND_FIRMWARE_UPDATE_READY) {
                                    ...
                                    int dataMaxLen = SmarthomeConnection.DATA_MAX_LENGTH - 4;   //или можно задавать меньшее число, в случае возникновения проблем при передаче
                                    for (int i = 0; i < hexReader.getLength(); i += pageSize) {
                                        ...
                                        for (int j = 0; j < len0; j += dataMaxLen) {
                                            ...
                                            r = connection.sendDataAndWaitResponse(device, SmarthomeCommand.BOOT_CONTROL, data,
                                    ...
                                    connection.sendData(device, SmarthomeCommand.BOOT_CONTROL, new byte[]{COMMAND_FIRMWARE_UPDATE_DONE});
                                    log("Done!");
